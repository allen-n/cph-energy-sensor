...
uint8_t CIRCUIT_PAIR[NUM_CIRCUITS] = {ADS8638_VOLT2, ADS8638_VOLT2,
                                      ADS8638_VOLT2, ADS8638_VOLT2,
                                      ADS8638_VOLT2, ADS8638_VOLT2};
...
// enum ACTIVE_CIRCUIT { MAIN1 = (int)ADS8638_CURR5, MAIN2 = (int)ADS8638_CURR6,
// BRANCH = (int)ADS8638_CURR1};
...
SampleBuf *samples[NUM_CIRCUITS] = {&samples_curr1,  &samples_curr2,
                                    &samples_branch, &samples_branch,
                                    &samples_branch, &samples_branch};

circuitVal circuit[NUM_CIRCUITS]; // circuit1(0.02, 1500); //NOTE: occupy 1.192
                                  // kB of mem
...
powerWave pWave_VOLT1(waveSize,
                      SAMPLE_RATE_VOLT1); // NOTE: Occupies 128 kB of mem
...
// String out = String(circuit) + " "; //indicating which circuit is sending the
// data out += c1.get_data_string();
void pushData(circuitVal &c1, bool &pushDataFlag) {
  int circuit = (int)circuit_state;
  if (circuit == 2)
    circuit += isrBranchCurrent;
  String out =
...
void logCircuit(waveform &iWave, waveform &vWave, powerWave &pWave,
  ...
  int circuit = (int)circuit_state;
  if (circuit == 2)
    circuit += isrBranchCurrent - 1;
  String out =
      String(circuit + 1) + " "; // indicating which circuit is sending the data
...
void transferBuff(SampleBuf &buff, bool &outFlag, waveform &vWave,
                  waveform &iWave) {
...
void timerLoop(State &state, powerWave &pWave, ACTIVE_CIRCUIT &circuit_state,
               bool &outFlag, circuitVal &circuit) {
  // Serial.print("Started timerLoop() for c");Serial.println(circuit_state);
  switch (state) {
  case STATE_INIT:
    switch (circuit_state) {
    case MAIN1:
      volt1_timer.begin(timerISR_VOLT1, 1000000 >> SAMPLE_RATE_SHIFT_VOLT1,
                        uSec, AUTO); // 122 uSec sample time
      break;
    case MAIN2:
      volt2_timer.begin(timerISR_VOLT2, 1000000 >> SAMPLE_RATE_SHIFT_VOLT2,
                        uSec, AUTO); // 122 uSec sample time
      break;
    default:
  ...
  case STATE_PROCESS:
    ...
    switch (circuit_state) { // once these values are finalized, put the final
    ...
    if (pWave.getPF() <= 0) {
      ...
      if (to_switch == BRANCH)
        to_switch += isrBranchCurrent - ADS8638_CURR1;
...
void loop() {
  ...
  // Serial.print("state_volt1:
  // ");Serial.print((int)MY_ADC.read1(ADS8638_CURR5)); Serial.print("
  // state_volt2: ");Serial.print((int)MY_ADC.read1(ADS8638_CURR6));
  // Serial.print(" state_branch:
  // ");Serial.println((int)MY_ADC.read1(ADS8638_CURR3));
}
...
void timerISR_VOLT1(void) {
  ...
  // Serial, String, any memory allocation (new, malloc, etc.), Particle.publish
  // and other Particle methods, and more.
  SampleBuf *sb = &samples_curr1;
  if (sb->free) {
    sb->i_data[sb->index] = MY_ADC.read1(ADS8638_CURR5);
    // sb->v_data[sb->index] = MY_ADC.read1(ADS8638_VOLT2); //account for sample
    // speed difference
...
void timerISR_BRANCH(void) {
  ...
  if (sb->free) {
    ...
    // sb->v_data[sb->index] = MY_ADC.read1(ADS8638_VOLT2); //account for sample
    // speed difference
    sb->v_data[sb->index] = MY_ADC.read1(
        CIRCUIT_PAIR[BRANCH + isrBranchCurrent -
                     ADS8638_CURR1]); // account for sample speed difference
    sb->t_data[sb->index] = micros();
