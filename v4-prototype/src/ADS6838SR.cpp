...
ads6838::ads6838() {
  ...
  // 	if(i%2 == 0) {
  // 		commandByte = (ADS8638_REG_MANUAL << 1) & 0xfe; //[15:9] reg addr,
  // [8] read/write 	} else {
  // 		// [7] always 0, [6:4] channel sel, [3:1] range sel, [0] temp
  // sel 		commandByte = k; k++; 		commandByte = commandByte << 4; 		commandByte =
  // commandByte & 0x70; 		uint8_t range = (ADS8638_RANGE_2_5V << 1) & 0x0e;
...
// 	std::memcpy(this->tx_buffer, this->tx_buffer_read8,
// sizeof(this->tx_buffer_read8)); 	transfer_state = 0x00;
// 	SPI.transfer(tx_buffer, rx_buffer, sizeof(rx_buffer),
// ads6838::trasnferHandler); 	flag = transfer_state;
// }
//
//
...
// 	// TODO: Add postprocessing to turn get the actual values out of the rx
// buffer 	std::memcpy(rx_dest, this->rx_buffer, sizeof(this->rx_buffer));
// }
...
void ads6838::selectChannel(uint8_t channel) {
  ...
  SPI.transfer((channel << 4) |
               (this->_range << 1)); // manual register addressing command
...
uint16_t ads6838::read1(uint8_t channel, uint8_t range) {
  ...
  digitalWrite(this->_SS, LOW);
  // delayMicroseconds(1);
  // while(!(SPI.available())); // better alternative to blocking code, may not
  // work
  SPI.transfer(0x00);
  SPI.transfer(0x00);
...
void ads6838::read8(uint16_t *out, uint8_t addr, uint8_t range) {
  ...
  for (uint8_t x = 0; x < 8; x++) { // FIXME: just checking probed curred1 track
    ...
    digitalWrite(this->_SS, LOW);
    // delayMicroseconds(1);
    // while(!(SPI.available())); // better alternative to blocking code, may
    // not work
    result[0] = SPI.transfer(0x00);
    result[1] = SPI.transfer(0x00);
